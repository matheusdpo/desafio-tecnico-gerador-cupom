...
public class GeradorCuponsController {
    ...
    @PostMapping("/create")
    public ResponseEntity<CouponResponse> create(@RequestBody(required = true) CouponRequest couponRequest) {
        ...
        return ResponseEntity.status(CREATED.value()).body(new CouponResponse("Your coupon has been created successfully"));
    }

    @DeleteMapping("/delete/{id}")
    public ResponseEntity<CouponResponse> delete(@PathVariable(required = true) Long id) {
        Optional<CouponEntity> couponEntityOptional = couponService.findByIdAndStatus(id, StatusCouponEnum.ACTIVE);
